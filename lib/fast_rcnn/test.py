...
from nms.rbox_nms import rbox_nms, prepare_nms
...
def im_detect(net, im, boxes=None):
    ...
    if cfg.TEST.HAS_RPN:
        ...
        boxes_front = rois[:, 1:5] / im_scales[0]
        theta = rois[:, 5].reshape(-1, 1)
        boxes = np.hstack((boxes_front, theta))
    ...
    if cfg.TEST.BBOX_REG:
        ...
        rbox_deltas = blobs_out['rbox_pred']
        pred_boxes = rbox_transform_inv(boxes, rbox_deltas)
        ...
        #print pred_boxes[:, 4:8]
        #print thetas[:, 1].shape
        #order = scores[:,1].argsort()[::-1]
        #print scores[order[:30],1].reshape(-1,1)
...
def test_net(net, imdb, max_per_image=100, thresh=0.05, vis=False):
    ...
    for i in xrange(num_images):
        ...
        for j in xrange(1, imdb.num_classes):
            ...
            '''
            keep = filter_boxes(cls_boxes, im.shape)
            cls_boxes = cls_boxes[keep,:]
            cls_scores = cls_scores[keep]
            '''
            pre_dets = prepare_nms(cls_boxes) 
            dets = np.hstack((pre_dets, cls_scores[:, np.newaxis])) \
            ...
            if dets.shape[0] == 0:
                ...
                keep = rotate_cpu_nms(dets, cfg.TEST.NMS, cfg.GPU_ID)
                cls_dets = np.hstack((cls_boxes, cls_scores[:, np.newaxis])) \
                .astype(np.float32, copy=False)
                cls_dets = cls_dets[keep, :]
                '''
                print '+++++++++++++++++++++++++++++++++++++++++'
                order = cls_dets[:, -1].argsort()[::-1]
                print cls_dets[order[:5],:]
                print '+++++++++++++++++++++++++++++++++++++++++'
                '''                     
            if vis:
